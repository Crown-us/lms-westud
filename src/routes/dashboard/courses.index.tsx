    ...
    <motion.div initial="hidden" animate="show" variants={container} className="max-w-[1400px] mx-auto space-y-8 pb-10">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 text-left">
        <div>
          ...
          <p className="text-slate-500 dark:text-slate-400 font-medium text-left">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-8 text-left">
        {filteredCourses.map((course: any) => (
          <motion.div key={course.id} variants={item}>
            ...
            >
              <Card className="group border-none shadow-sm hover:shadow-2xl transition-all duration-500 rounded-[2.5rem] overflow-hidden bg-white dark:bg-slate-900 h-full flex flex-col text-left">
                <div className="relative aspect-[16/10] overflow-hidden text-left">
                ...
                <CardContent className="p-7 flex-1 space-y-4 text-left">
                  <div className="flex justify-between items-center text-left">
                    <span className="text-[10px] font-black uppercase tracking-widest text-red-500">{course.category}</span>
                    <div className="flex items-center gap-1 text-yellow-500 font-black text-xs text-left">
                      <Star className="w-3.5 h-3.5 fill-current" /> {course.rating}
                    </div>
                  </div>
                  
                  <h3 className="text-xl font-black text-slate-800 dark:text-white leading-tight line-clamp-2 text-left min-h-[3.5rem]">
                    {course.title}
                  </h3>
                  
                  {isTeacher ? (
                    <div className="grid grid-cols-2 gap-2 pt-4 border-t border-slate-50 dark:border-slate-800/50 text-left">
                       <div className="space-y-1 text-left overflow-hidden">
                          <div className="text-[9px] font-black text-slate-400 uppercase tracking-widest text-left truncate">Total Murid</div>
                          <div className="font-black text-slate-800 dark:text-white flex items-center gap-1.5 text-left">
                             <Users className="w-3.5 h-3.5 text-red-500 shrink-0" /> 
                             <span className="text-base truncate">{course.total_students || 0}</span>
                          </div>
                       </div>
                       <div className="space-y-1 text-left overflow-hidden">
                          <div className="text-[9px] font-black text-slate-400 uppercase tracking-widest text-left truncate">Harga</div>
                          <div className="font-black text-green-600 flex items-center gap-1 text-left">
                    ...
                    <div className="space-y-3 pt-4 border-t border-slate-50 dark:border-slate-800/50 text-left">
                       <div className="flex justify-between text-[10px] font-black text-slate-400 uppercase tracking-widest text-left">
                ...
                <CardFooter className="p-7 pt-0 flex gap-2 text-left">
                   {isTeacher ? (
                     <>
